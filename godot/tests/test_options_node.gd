...
func _ready():
	var ok := true
	# ── Opzione A (auto): scarta 1 + pesca 2; niente stordimento, kamae invariata ──
	var s := GameState.new()
	...
	elif f.hand.size() != hand0 - 1 + 2:
		print("FAIL: opzione A mano attesa %d, ottenuta %d" % [hand0-1+2, f.hand.size()]); ok=false
	else:
		print("OK: opzione A → scarta 1 + pesca 2 (mano %d→%d), nessuno stordimento, kamae invariata" % [hand0, f.hand.size()])

	# ── Opzione B (scelta): +1 stordito + passa a kamae qualsiasi; niente pesca/scarto ──
	...
	duel2.set_option_choice(0, "b")
	var hand2 := f2.hand.size()
	duel2._resolve_card(0, {}, [])
	if f2.stun != 1:
		print("FAIL: opzione B dovrebbe dare +1 stordimento (stun=%d)" % f2.stun); ok=false
	elif Domain.STANCE_SLUG[f2.stance] == "neutral":
		print("FAIL: opzione B dovrebbe cambiare kamae"); ok=false
	elif f2.hand.size() != hand2:
		print("FAIL: opzione B non dovrebbe pescare/scartare (mano %d→%d)" % [hand2, f2.hand.size()]); ok=false
	else:
		print("OK: opzione B → +1 stordito, kamae %s, mano invariata" % Domain.STANCE_SLUG[f2.stance])
